...
#include "rtlmemmgr_malloc.h"

using namespace SST::rtlComponent;

rtlMemoryManagerMalloc::rtlMemoryManagerMalloc(ComponentId_t id, Params& params) :
    ...
    for (uint32_t i = 0; i < memoryLevels; ++i) {
        ...
        if (rtlPageMappingPolicy::LINEAR == mapPolicy) {
...
rtlMemoryManagerMalloc::~rtlMemoryManagerMalloc() {
}


void rtlMemoryManagerMalloc::setDefaultPool(uint32_t pool) {
    defaultLevel = pool;
}

uint32_t rtlMemoryManagerMalloc::getDefaultPool() {
    return defaultLevel;
}
 ...
 */
bool rtlMemoryManagerMalloc::canAllocateInLevel(const uint64_t size, const uint32_t level) {
    int pageCount = size / pageSizes[level] + ((size % pageSizes[level] == 0) ? 0 : 1);
    return freePages[level]->size() >= pageCount;
}


void rtlMemoryManagerMalloc::allocate(const uint64_t size, const uint32_t level, const uint64_t virtualAddress) {
 ...
 *
 *  Therefore, rtl should immediately translate virtual to physical addresses and ALWAYS use physical addresses when computing offsets.
 */
bool rtlMemoryManagerMalloc::allocateMalloc(const uint64_t size, const uint32_t level, const uint64_t virtualAddress, const uint64_t instructionPointer, const uint32_t thread) {
    ...
    // Allocate new page(s). Round malloc to nearest whole page TODO fix so we can map partial pages -> needs a local VA->rtl_VA mapping
...
void rtlMemoryManagerMalloc::freeMalloc(const uint64_t virtualAddress) {
...
uint64_t rtlMemoryManagerMalloc::translateAddress(uint64_t virtAddr) {
...
void rtlMemoryManagerMalloc::printStats() {
    output->output("\n");
